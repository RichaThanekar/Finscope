...
# # Last actual inflation
# latest_inflation = df_prophet['y'].iloc[-1]
# # Next month forecast
# next_month_forecast = forecast.iloc[len(df_prophet)]['yhat']
# # Avg forecast over next 12 months
# avg_forecast_12m = forecast.tail(12)['yhat'].mean()

# # Example: user’s current monthly expense
# current_expense = st.number_input("Enter your current monthly expense (₹):", value=50000, step=1000)

# # Future expense after 1 year
# future_expense = current_expense * (1 + avg_forecast_12m/100)

# # Purchasing power erosion (₹1L today in 1 year)
# erosion_value = 100000 / (1 + avg_forecast_12m/100)

# # Show as KPIs
# col1, col2, col3 = st.columns(3)
# col1.metric("📌 Latest Inflation", f"{latest_inflation:.2f}%")
# col2.metric("📅 Next Month Forecast", f"{next_month_forecast:.2f}%")
# col3.metric("📊 Avg 12M Forecast", f"{avg_forecast_12m:.2f}%")

# st.markdown(f"""
# - 💸 **Purchasing Power Erosion:** ₹1,00,000 today will be worth **₹{erosion_value:,.0f}** after 1 year.  
# - 🏠 **Household Expense Growth:** Your monthly expense of **₹{current_expense:,.0f}** may rise to **₹{future_expense:,.0f}** next year.  
# - 🔮 **Scenarios:** Your expense could range between **₹{current_expense*(1+forecast.tail(12)['yhat_lower'].mean()/100):,.0f} (optimistic)** and **₹{current_expense*(1+forecast.tail(12)['yhat_upper'].mean()/100):,.0f} (pessimistic)**.  
# """)
...
# fig.add_trace(go.Scatter(
#     x=df_prophet['ds'], y=df_prophet['Fitted'],
#     mode='lines', name='Fitted (Prophet)', line=dict(color='green', dash='dash')
# ))

# # Forecast
# fig.add_trace(go.Scatter(
#     x=forecast['ds'], y=forecast['yhat'],
#     mode='lines+markers', name='Forecast (Prophet)', line=dict(color='red')
# ))
...
# fig.update_layout(
#     title='Combined Inflation: Actual, Fitted & Forecast',
#     xaxis_title='Month',
#     yaxis_title='Combined Inflation (%)',
#     hovermode='x unified',
#     height=600,
...
# # 8️⃣ Forecast Table
# # -------------------------------
# st.subheader("📅 Forecasted Combined Inflation (Next 12 Months)")
# forecast_12m = forecast[['ds', 'yhat', 'yhat_lower', 'yhat_upper']].tail(12)
# forecast_12m = forecast_12m.rename(columns={
#     'ds': 'Month',
#     'yhat': 'Predicted',
